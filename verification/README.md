...
It scores every agent against its own gold set (S4.3.4, ADR 0040, product spec Section 11): `astra-verify agent-eval score --gold agents/<agent>/eval.yaml --predictions agents/<agent>/predictions.yaml` compares the agent's predicted output items with the gold set's expected ones case by case, rolls precision and recall up by tier (simple/medium/complex, micro-averaged across the tier's cases, not the mean of each case's own rate), checks each against the tier's own threshold, and fails — blocking release — on a tier below threshold or a gold set case with no prediction at all. `astra-verify agent-eval report [agents]` scores every agent that has both a gold set and predictions present and publishes one report by tier across every agent, the weekly evidence a scheduled workflow runs. Needs no Snowflake connection; the harness only reads files.
