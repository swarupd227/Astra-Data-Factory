...
| `astra-data render [--out releases] [--check] [paths]` | Compiles each config and renders every artifact into `releases/<bundle>/`: Bronze DDL, pipeline SQL (lines view, intake, process, tasks), data metric functions, tests, docs, an Atlan payload and PROVENANCE.json. Byte-identical for an unchanged config; `--check` fails when a committed bundle is stale ([ADR 0018](../docs/adr/0018-release-bundle-rendering.md)) | nothing |
...
src/astra_data/
  ...
  render/                          renderers per artifact: bronze (DDL, lines view, intake, process), tasks, dq, tests, docs, atlan; bundle assembly and provenance
